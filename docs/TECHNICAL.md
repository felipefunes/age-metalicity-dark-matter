...
Este documento contiene todo el detalle de implementación del proyecto: estado real de los
datos, hallazgos, stack, estructura del repo, esquema de la base de datos, cómo correr el
pipeline y el frontend, despliegue y tests. Si buscás una introducción de qué es el proyecto y
qué tan confiable es cada parte, ver [`README.md`](../README.md).
...
|---|---|---|
| 2026-08-22 | [`hubble_mass_dm_v1`](findings/2026-08-22_hubble_mass_dm_v1.md) — masa como confounder casi total del tipo de Hubble; T–f_DM se invierte al controlar por masa | **Vigente** — conclusiones sin cambios |
| 2026-08-22 | [`hubble_mass_dm_v2_log_control_check`](findings/2026-08-22_hubble_mass_dm_v2_log_control_check.md) — verificación: ¿hace falta controlar por log-masa en vez de masa cruda? (no, ver documento) | **Vigente** — confirma v1, no lo reemplaza |
| 2026-08-22 | [`metallicity_age_proxy_v1`](findings/2026-08-22_metallicity_age_proxy_v1.md) — primera integración de metalicidad real (Moustakas+2010, Pilyugin+2014) y proxy de edad por sSFR (z0MGS); correlaciones con f_DM | **Vigente** — preliminar/indicativo por muestra chica en metalicidad (n=22), no concluyente |
| 2026-08-23 | [`stellar_age_gallazzi_attempt`](findings/2026-08-23_stellar_age_gallazzi_attempt.md) — intento de edad estelar vía Gallazzi+2005 (SDSS DR4); n=13, **no integrado** | Vigente — registro de por qué no se usó esta vía |
| 2026-08-23 | [`dn4000_hdelta_a`](findings/2026-08-23_dn4000_hdelta_a.md) — Dn4000 y Hδ_A medidos directamente de espectros SDSS (n=41), validados contra el pipeline oficial MPA-JHU; incluye la sección de referencia sobre la degeneración edad-metalicidad | **Vigente** — preliminar (n=41), ninguna correlación con f_DM concluyente |
...
- **`metallicity_age`**`(pgc_id FK, metallicity, metallicity_source, metallicity_method, age_gyr,
  age_source, age_method, metallicity_kk04, e_metallicity_kk04, metallicity_pt05,
  e_metallicity_pt05, n_hii_regions_moustakas, metallicity_pilyugin2014,
  e_metallicity_pilyugin2014, age_proxy_ssfr, e_age_proxy_ssfr, age_proxy_source,
  age_proxy_method, age_proxy_dn4000, e_age_proxy_dn4000, n_pixels_dn4000, age_proxy_hdelta_a,
  e_age_proxy_hdelta_a, n_pixels_hdelta_a)` — `metallicity`/`age_gyr` son el slot genérico de
  HyperLeda (siempre nulo hoy, ver arriba); el resto son las columnas por fuente añadidas en PRs
  posteriores, cada una con su propia incertidumbre y, cuando aplica, su propio contador de
  regiones/píxeles usados (`n_hii_regions_moustakas`, `n_pixels_dn4000`, `n_pixels_hdelta_a`).
